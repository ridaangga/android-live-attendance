...
class ForgotPasswordActivity : AppCompatActivity() {
    ...
    private fun onClick() {
        ...
        binding.btnForgotPassword.setOnClickListener {
            ...
            if (isFormValid(email)){ //Jika email valid, jalankan class isFormValid yang ada di bawah
                forgotPassToServer(email) //jalankan class forgotPassToServer yang ada di bawah
            }
        }
    }
    ...
    private fun forgotPassToServer(email: String) {
        ...
        ApiServices.getLiveAttendanceServices()
            ...
            .enqueue(object : Callback<ForgotPasswordResponse> { //callback pilih yang retrofit2,
                override fun onResponse(
                    ...
                    if (response.isSuccessful){
                        val message = response.body()?.message //dapatkan pesan dari body
                        MyDialog.dynamicDialog(
                            this@ForgotPasswordActivity,
                            getString(R.string.success),
                            message.toString()
                        )
                        Handler(Looper.getMainLooper()).postDelayed({ //ppakai handler jadi tidak langsung menghilang dialognya.
                            MyDialog.hideDialog()
                            finish()
                        },2000)
                    }else{
                        val errorConverter: Converter<ResponseBody, ForgotPasswordResponse> = //converternya di alt+string pakai yang retrofit.
                            RetrofitClient
                                .getClient()
                        ...
                        }catch (e: IOException){ //IOException pake yang java.io
                            e.printStackTrace()
    ...
    private fun isFormValid(email: String): Boolean {
        if (email.isEmpty()){ //jika email yang diisikan kosong
            binding.etEmailForgotPassword.error = getString(R.string.please_field_your_email)
            binding.etEmailForgotPassword.requestFocus()
        }else if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()){ //jika email tidak valid
            binding.etEmailForgotPassword.error = getString(R.string.please_use_valid_email)
            binding.etEmailForgotPassword.requestFocus()
        }else{
            binding.etEmailForgotPassword.error = null //jika tidak error, erronya null kan.
            return true
        }
        return false
    }
